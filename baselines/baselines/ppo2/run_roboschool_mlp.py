...
def test(env_id, num_timesteps, seed, curr_path, point):
    ...
    running_mean = np.load('{}/ppo2-mlp-{}-log/mean.npy'.format(curr_path, env_id))
    running_var = np.load('{}/ppo2-mlp-{}-log/var.npy'.format(curr_path, env_id))
    ...
    ppo2.test(policy=policy, env=env, nsteps=2048, nminibatches=32,
        load_path='{}/ppo2-mlp-{}-log/checkpoints/{}'.format(curr_path, env_id, point))
...
def main():
    ...
    if args.train:
        logger.configure(dir='{}/ppo2-mlp-{}-log'.format(curr_path,args.env))
