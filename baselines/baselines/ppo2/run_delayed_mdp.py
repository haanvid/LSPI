...
def test(env_id, num_timesteps, seed, curr_path, point, delay_steps, np_op_act):
    ...
    running_mean = np.load('{}/log/mean.npy'.format(curr_path,delay_steps))
    running_var = np.load('{}/log/var.npy'.format(curr_path, delay_steps))
    ...
    ppo2.test(policy=policy, env=env, nsteps=2048, nminibatches=32, 
        load_path='{}/log/checkpoints/{}'.format(curr_path, delay_steps, point))
...
def main():
    ...
    if args.train:
        logger.configure(dir='{}/log'.format(curr_path, args.delayed))
